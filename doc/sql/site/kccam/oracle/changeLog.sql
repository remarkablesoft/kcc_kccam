...
-- 2023.04.28 개선 사항 처리 윤건희
1. TS02_TB_POPUP (Remarkable_CREATE_BOARD_TABLE.sql 참조 )
2. 제품명과 설명에서 -EN -CN 제거
UPDATE TS02_TB_CATEGORY_LANG
   SET DESCR = REPLACE(DESCR, '-EN', '')
 WHERE INSTR(descr, '-EN') > 0; --128

UPDATE TS02_TB_CATEGORY_LANG
   SET NAME = REPLACE(NAME, '-EN', '')
 WHERE INSTR(NAME, '-EN') > 0;  --115

UPDATE TS02_TB_CATEGORY_LANG
   SET DESCR = REPLACE(DESCR, '-CN', '')
 WHERE INSTR(DESCR, '-CN') > 0;  -- 128

UPDATE TS02_TB_CATEGORY_LANG
   SET NAME = REPLACE(NAME, '-CN', '')
 WHERE INSTR(NAME, '-CN') > 0;

3. AlN 류 제품 명에서 숫자 제거
UPDATE TS02_TB_CATEGORY
SET NAME = REGEXP_REPLACE(NAME, '\d', '') where name like '%AlN%';
...
UPDATE TS02_TB_CATEGORY_LANG
   SET NAME = '半導體'
 WHERE OID = '1SUXU0TR000';
   AND LANG = 'CN';

UPDATE TS02_TB_CATEGORY_LANG
   SET NAME = 'General Industry'
 WHERE OID = '1SUXU0TR007'
   AND LANG = 'EN';

UPDATE TS02_TB_CATEGORY_LANG
SET NAME = '電器/電子設備'
WHERE OID = '1SUXU0TR003'
  AND LANG = 'CN';
